...
declare const __karma__: {
  config: { args: [Array<{ provider: string; port: number }>] };
};

const getPactServiceConfiguration = (
  providerName: string
): { provider: string; port: number } | undefined =>
 ...
 */
const _describePact = (
  describeFunctionToBeUsed: (
    _description: string,
    _specDefinitions: () => void
  ) => void,
  description: string,
  providerName: string,
